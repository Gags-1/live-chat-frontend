...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!token) {
        ...
        // Decode the JWT to get the user's ID (username)
        const payload = JSON.parse(atob(token.split('.')[1]));
        const username = payload.username;  // Assuming you include 'username' in the token's payload
        ...
        var ws = new WebSocket(`ws://127.0.0.1:8000/ws/${username}`);
